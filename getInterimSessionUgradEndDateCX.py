# returns the end date of the current PeopleSoft interim session
...
def getInterimSessionUgradEndDateCX (self):
    ...
    if remote_addr in [Plone1, CMF2, Kim_Nguyen_Air, Kim_Nguyen_G5, Plone3, '127.0.0.1', ]:
        ...
        cursor.execute("""select sess_end_dt from ps_session_tbl where sess_begin_dt <= sysdate and sess_end_dt >= sysdate and acad_career = 'UGRD' and session_code = '3WI'""")
        for column_1 in cursor:
            try:
                return column_1[0]
            except:
                return "not a 3WI session"
        return "not a 3WI session"
